          ...
          {filtered.map(p => (
            <div key={p.uid} style={{
              ...
              <button
                onClick={() => {
                  if (onOpenMessage) {
                    const playerName = ((p.firstName || "") + " " + (p.lastName || "")).trim() || "Player";
                    onOpenMessage(p.uid, playerName, "player", activeProgram?.id);
                  }
                }}
                style={{
                  marginTop: 12, width: "100%", padding: "8px 16px", borderRadius: 8,
                  border: "none", background: "#0A1F44", color: "#fff",
                  fontWeight: 600, fontSize: 13, cursor: "pointer",
                  transition: "background 0.15s",
                }}
                onMouseEnter={e => e.currentTarget.style.background = "#122d5e"}
