/**
 * School reference data — city, state, NCAA division, school type, enrolment.
 *
 * Extracted from enrich-schools.js so the sync can use it directly.  That file
 * reads merged-programs.json at module load, so importing it ran the whole
 * script as a side effect, which is why the enrichment it provides had never
 * been wired into sync.js.
 *
 * The scrapers that supply state are Next Phase and the conference sites; when
 * Next Phase is unavailable this table is the only thing standing between a
 * newly discovered club and firestore-sync's "new program without state"
 * rejection.
 */
...
const BY_NORM = new Map(Object.entries(SCHOOL_INFO).map(([k, v]) => [norm(k), v]));

/** Reference row for a school name, or null. */
export function lookupSchool(school) {
  return BY_NORM.get(norm(school)) || null;
}

/**
 * Fill blank fields on merged programs from the reference table.
 *
 * Only fills what is missing — a scraped value always wins over the table,
 * matching the rule the rest of the pipeline follows.
 * Returns { programs, filled, byField }.
 */
export function enrichPrograms(programs) {
  const FIELDS = ["city", "state", "ncaaDivision", "schoolType", "enrollment"];
  const byField = {};
  let filled = 0;

  const out = programs.map(p => {
    const info = lookupSchool(p.school);
    if (!info) return p;

    const next = { ...p };
    for (const f of FIELDS) {
      const empty = next[f] === undefined || next[f] === null || next[f] === "";
      if (empty && info[f] !== undefined && info[f] !== "") {
        next[f] = info[f];
  ...
  return { programs: out, filled, byField };
}

export { SCHOOL_INFO, isJunk };
